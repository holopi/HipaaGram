...
#else
    [ProxyHTTPManager doPost:@"/auth/signin" withParams:@{@"username":username, @"usersId":usersId, @"phoneNumber":phoneNumber, @"password":password} block:block];
#endif
}
...
#else
    [ProxyHTTPManager doPost:[NSString stringWithFormat:@"/message/%@/%@",[[NSUserDefaults standardUserDefaults] valueForKey:kPhoneNumber], username] withParams:nil block:block];
#endif
}
...
#else
    [ProxyHTTPManager doGet:[NSString stringWithFormat:@"/message/%@",[[NSUserDefaults standardUserDefaults] valueForKey:kPhoneNumber]] block:block];
#endif
}
...
#else
    [ProxyHTTPManager doGet:@"/contacts" block:block];
#endif
}
